...
fn main() {
    ...
    let mut overlay_tx_opt = Some(spawn_overlay_window(Arc::clone(&shared_metrics)));
            ...
            Event::MainEventsCleared => {
                while let Ok(ev) = menu_rx.try_recv() {
                    if ev.id == switch_item.id() {
                        ...
                        *m = match *m { Mode::CpuRam => Mode::DiskNet, Mode::DiskNet => Mode::CpuRam };
                        ...
                        if let Some(tx) = overlay_tx_opt.take() {
                            ...
                            overlay_tx_opt = Some(spawn_overlay_window(Arc::clone(&shared_metrics)));
...
#[cfg(windows)]
fn spawn_overlay_window(shared: Arc<Mutex<Metrics>>) -> mpsc::Sender<OverlayCmd> {
    use std::mem::zeroed;
    ...
    thread::spawn(move || unsafe {
        ...
        extern "system" fn wndproc(hwnd: HWND, msg: u32, _w: WPARAM, _l: LPARAM) -> LRESULT {
            unsafe {
                match msg {
                    WM_PAINT => {
                        ...
                        if !ptr.is_null() {
                            let m = *ptr;
                            let mut y = 6;
                            draw_text_shadowed(hdc, 8, y, "--System Usage--"); y += 20;
                            draw_text_shadowed(hdc, 8, y, &format!("CPU: {:.1} %", m.cpu_pct)); y += 18;
                            draw_text_shadowed(hdc, 8, y, &format!("RAM: {:.1} %", m.ram_pct)); y += 18;
                            draw_text_shadowed(hdc, 8, y, &format!("DISK: {:.2} MB/s ({:.0} %)", m.disk_mbps, m.disk_pct)); y += 18;
                            draw_text_shadowed(hdc, 8, y, &format!("NET: {:.0} KB/s ({:.0} %)", m.net_kbps, m.net_pct));
                        }
        ...
        unsafe fn draw_text_shadowed(
            ...
            use windows_sys::Win32::Graphics::Gdi::{TextOutW, SetTextColor};
        ...
        // 2) 창 생성 (좌상단, 테두리 없음, 항상 위, 툴윈도우, 레이어드)
        let hwnd = CreateWindowExW(
            WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_LAYERED | WS_EX_TRANSPARENT, // ★ 클릭 스루
            class_name.as_ptr(),
            class_name.as_ptr(),
            WS_POPUP,
            8, 8, 240, 120,
...
#[cfg(not(windows))]
fn spawn_overlay_window(_shared: Arc<Mutex<Metrics>>) -> mpsc::Sender<OverlayCmd> {
